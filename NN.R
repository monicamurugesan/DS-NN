...
table(predicted_strength,concrete_test$strength)
# predicted_strength
# model_results$neurons
cor(predicted_strength,concrete_test$strength)

plot(predicted_strength,concrete_test$strength)
model_5<-neuralnet(strength~cement+slag+ash+water+superplastic+coarseagg+fineagg+age,data= concrete_norm,hidden = 5,threshold=0.04, act.fct="tanh", linear.output=TRUE, stepmax=1e7)
...
model_t <-neuralnet(strength~cement+slag+ash+water+superplastic+coarseagg+fineagg+age,data= concrete_norm,threshold=0.07,hidden=10,act.fct="logistic",linear.output = TRUE,stepmax=1e7)

model_t_res<-compute(model_t,concrete_test[1:8])
predict <-model_t_res$net.result
cor(predict,concrete_test$strength)
table(predict,concrete_test$strength)
plot(predict,concrete_test$strength)
